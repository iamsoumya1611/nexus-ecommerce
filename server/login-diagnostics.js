// Diagnostic script to test login functionality in isolation
...
app.get('/test', (req, res) => {
  res.json({ message: 'Login diagnostics server is running' });
});
...
app.post('/test-password-compare', async (req, res) => {
  try {
    ...
    const user = await User.findOne({ email });
    if (!user) {
      logger.warn('User not found for password comparison');
...
app.listen(PORT, () => {
  logger.info(`Login diagnostics server running on port ${PORT}`);
  logger.info('Test endpoints:');
  logger.info('  GET  /test - Test if server is running');
